...
void MSD<T>::reset(const unsigned int numeroTimestepsPerBlocco) {
    ...
    lunghezza_lista=leff*ntypes*f_cm*f_corr;
...
void MSD<T>::calcola(unsigned int primo) {
    ...
    if (nthread<1) {
        unsigned int *cont=new unsigned int [ntypes*f_cm*f_corr];

        for (unsigned int t=0;t<leff;t++) {
            for (unsigned int i=0;i<ntypes*f_cm*f_corr;i++){
                lista[ntypes*t*f_cm*f_corr+i]=0.0;
                cont[i]=0;
            }
            for (unsigned int imedia=0;imedia<ntimesteps;imedia+=skip){
                for (unsigned int iatom=0;iatom<traiettoria->get_natoms();iatom++) {
                    double delta=(pow(traiettoria->posizioni(primo+imedia,iatom)[0]-traiettoria->posizioni(primo+imedia+t,iatom)[0],2)+
                            ...
                            -lista[ntypes*t*f_cm*f_corr+traiettoria->get_type(iatom)];
                    lista[ntypes*t*f_cm*f_corr+traiettoria->get_type(iatom)]+=delta/(++cont[traiettoria->get_type(iatom)]);
        ...
        */
                    ...
                    for (unsigned int imedia=0;imedia<ntimesteps;imedia+=skip){
                        ...
                        if (cm_msd) {
                            ...
                            double delta=(pow(traiettoria->posizioni_cm(primo+imedia,itype)[0]-traiettoria->posizioni_cm(primo+imedia+t,itype)[0],2)+
                                    ...
                                    -lista[ntypes*t*f_cm*f_corr+ntypes+itype];
						...
						if (cross_msd) {
                            for (unsigned int iatom=0;iatom<traiettoria->get_natoms();iatom++) {
                            	for (unsigned int jatom=0;jatom<traiettoria->get_natoms();jatom++) {
									...
									double delta = (dx2ij + dy2ij + dx2ij) - lista[ntypes*t*f_cm*f_corr+traiettoria->get_type(iatom)];
                                	lista[ntypes*t*f_cm*f_corr+traiettoria->get_type(iatom)] += delta/(++cont[traiettoria->get_type(iatom)]);
								}
							}
						}
        ...
        if (debug) {
            ...
            for (unsigned int ts=0;ts<leff;ts++) {
                out << ts;
                for (unsigned int itype=0;itype<ntypes*f_cm*f_corr;itype++){
                    out <<" "<<lista[ntypes*ts*f_cm*f_corr+itype];
                }
                out << "\n";
            }
            out << "\n\n";
